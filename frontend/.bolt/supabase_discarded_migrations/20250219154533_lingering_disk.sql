...
BEGIN
  ...
  VALUES ('أحمد محمد علي', 'ahmed@example.com', '+20 123 456 789', 'القاهرة، مصر', 'individual', lawyer_id)
  RETURNING id INTO client_id1;

  INSERT INTO clients (name, email, phone, address, type, lawyer_id)
  VALUES ('شركة الأمل التجارية', 'info@alamal.com', '+20 111 222 333', 'الإسكندرية، مصر', 'company', lawyer_id)
  RETURNING id INTO client_id2;

  INSERT INTO clients (name, email, phone, address, type, lawyer_id)
  VALUES ('محمد أحمد حسن', 'mohamed@example.com', '+20 999 888 777', 'الجيزة، مصر', 'individual', lawyer_id)
  RETURNING id INTO client_id3;

  INSERT INTO clients (name, email, phone, address, type, lawyer_id)
  VALUES ('شركة النور للمقاولات', 'info@alnour.com', '+20 444 555 666', 'القاهرة، مصر', 'company', lawyer_id)
  RETURNING id INTO client_id4;

  -- Insert cases
  INSERT INTO cases (title, type, court, status, client_id, lawyer_id)
  VALUES ('قضية تعويض مدني', 'مدني', 'محكمة القاهرة الابتدائية', 'open', client_id1, lawyer_id)
  RETURNING id INTO case_id1;

  INSERT INTO cases (title, type, court, status, client_id, lawyer_id)
  VALUES ('نزاع عقود تجارية', 'تجاري', 'محكمة الإسكندرية الاقتصادية', 'in_progress', client_id2, lawyer_id)
  RETURNING id INTO case_id2;

  INSERT INTO cases (title, type, court, status, client_id, lawyer_id)
  VALUES ('قضية عقارية', 'عقاري', 'محكمة الجيزة الابتدائية', 'open', client_id3, lawyer_id)
  RETURNING id INTO case_id3;

  INSERT INTO cases (title, type, court, status, client_id, lawyer_id)
  VALUES ('نزاع عمل', 'عمالي', 'محكمة العمل', 'closed', client_id4, lawyer_id)
  ...
  RETURNING id INTO invoice_id3;

  -- Insert invoice items
  INSERT INTO invoice_items (description, amount, invoice_id)
  VALUES 
    ('استشارة قانونية', 2500, invoice_id1),
    ('إعداد مستندات', 2500, invoice_id1),
    ('صياغة عقد', 4000, invoice_id2),
    ('تمثيل قانوني', 4000, invoice_id2),
    ('استشارة قانونية', 1500, invoice_id3),
    ('إعداد مستندات', 1500, invoice_id3);
